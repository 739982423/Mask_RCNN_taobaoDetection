...
class BalloonDataset(utils.Dataset):        #数据读取类
    ...
    def load_balloon(self, dataset_dir, subset):
        ...
        for a in annotations:
            ...
            if type(a['regions']) is dict:
                polygons = [r['shape_attributes'] for r in a['regions'].values()]       #polygons:多边形，即数据集中的box（物体的边缘线）
            else:
            ...
            image = skimage.io.imread(image_path)                           #函数作用是从文件目录下加载图片,返回numpy.ndarray对象，
                                                                            #通道顺序为RGB，通道值默认范围0-255。
    ...
    def load_mask(self, image_id):                  #该函数在model中被调用，传入参数只有image_id，需要返回该id的image的mask和class
       ...
       Returns:
        ...
        info = self.image_info[image_id]                                   #拿到该image的info，info中包含了self.add_image时填入的信息
